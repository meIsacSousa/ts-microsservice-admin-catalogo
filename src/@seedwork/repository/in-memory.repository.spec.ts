...
describe("InMemoryRepository Unit Tests", () => {
    ...
    it("Should throws an error when find by id with entity not found", async () => {
        ...
        await expect(repository.findById(new UniqueEntityId(uuid))).rejects
            .toThrowError(`Entity with id ${uuid} not found`);
        await expect(repository.findById("not-found")).rejects
            .toThrowError("Entity with id not-found not found");
    });
    ...
    it("Should throws an error on update with entity not found", async () => {
        ...
        await expect(repository.update(entity)).rejects
            .toThrowError(`Entity with id ${entity.id} not found`);
    });
    ...
    it("Should update an entity", async () => {
        ...
        const entityUpdated = new StubEntity({ name: "Test 2", price: 20 }, entity.uniqueEntityId);
    ...
    it("Should throws an error on delete with entity not found", async () => {
        const uuid = "b650511d-ab7d-49a8-97fd-a0d1253c8ff5";
        await expect(repository.delete(new UniqueEntityId(uuid))).rejects
            .toThrowError(`Entity with id ${uuid} not found`);
        await expect(repository.delete(uuid)).rejects
            .toThrowError(`Entity with id ${uuid} not found`);
    });
