  ...
  describe("Create Method", () => {
    it("Should get invalid to category when create", () => {
      const asserts = [
        ...
        {
          props: { name: "name", description: true },
          errors: { description: ["description must be a string"] },
        },
        {
          props: { name: "name", description: 1 },
          errors: { description: ["description must be a string"] },
        },
        {
          props: { name: "name", description: { a: "object" } },
          errors: { description: ["description must be a string"] },
        },
        {
          props: { name: "name", is_active: "invalid" },
          errors: { is_active: ["is_active must be a boolean value"] },
        },
        {
          props: { name: "name", is_active: 1 },
          errors: { is_active: ["is_active must be a boolean value"] },
        },
        {
          props: { name: null, description: true, is_active: 1 },
          errors: {
            name: [
              "name should not be empty",
              "name must be a string",
  ...
  describe("Update Method", () => {
    it("Should get invalid to category when update", () => {
      ...
      const asserts = [
        ...
        {
          props: { name: null, description: { a: "object" } },
          errors: {
            name: [
              "name should not be empty",
              "name must be a string",
        ...
        {
          props: { name: 1, description: { a: "object" } },
          errors: {
            name: [
              "name must be a string",
              "name must be shorter than or equal to 255 characters",
      ...
      asserts.forEach((assert) => {
        expect(() =>
          category.update(
            assert.props.name as any,
            assert.props.description as any
          )
        ).containsErrorMessages({ ...assert.errors });
      });
    });
